class InstalledSoftware < ApplicationRecord
  ...
  validate :check_finish_date_required
  ...
  def check_finish_date_required
    if perpetual_flag == false && finish_date.blank?
      errors.add(:finish_date, "должно быть заполнено, если лицензия не бессрочная")
    end
  end
end